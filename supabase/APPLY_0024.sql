...
--
-- Fixes the lounge not updating until you refresh.
--
-- chat_messages was never added to the supabase_realtime
-- publication, so Postgres published nothing and the browser's
-- subscription sat there receiving silence. Short, and safe to
-- run twice.
-- =============================================================
...
select 'chat_messages is published' as thing,
       exists (
         select 1 from pg_publication_tables
         where pubname = 'supabase_realtime'
           and schemaname = 'public'
